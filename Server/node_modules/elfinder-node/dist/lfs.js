...
const helpers = __importStar(require("./lfs.utils"));
...
const LFS = (options) => {
    ...
    return {
        ...
        dim: async function (opts) {
            const target = helpers.decode(opts.target, config);
            const img = await jimp_1.default.read(target.absolutePath);
            return {
        ...
        extract: async function (opts) {
            ...
            const tasks = files.map(async (file) => helpers.info(path.resolve(dest, file), config));
            return { added: await Promise.all(tasks) };
        },
        file: async function (opts, res) {
            const target = helpers.decode(opts.target, config);
        ...
        open: async function (opts) {
            ...
            if (!dirExists)
                target = helpers.decode(encodedRoot, config);
            let files = (await fs.readdir(target.absolutePath).catch(console.log)) || [];
            const tasks = files.map(async (file) => helpers.info(path.join(target.absolutePath, file), config));
        ...
        parents: async function (opts) {
            if (!opts.target)
                throw new Error('errCmdParams');
            const dir = helpers.decode(opts.target, config);
            let tree = await helpers.init(config);
            const read = async (t) => {
        ...
        resize: async function (opts) {
            const target = helpers.decode(opts.target, config);
            let image = await jimp_1.default.read(target.absolutePath);
            if (opts.mode == 'resize') {
        ...
        tmb: async function (opts) {
            ...
            const tasks = [];
            _.each(files, (file) => {
                tasks.push(jimp_1.default.read(file).then(async (img) => {
